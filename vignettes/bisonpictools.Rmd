...
dir <- "myfilepath"
event_data <- readr::read_csv(
  file.path(dir, "event_data.csv")
)
location_data <- readr::read_csv(
  file.path(dir, "location_data.csv")
)
census_data <- readr::read_csv(
  file.path(dir, "census_data.csv")
)
proportion_calf_data <- readr::read_csv(
  file.path(dir, "proportion_calf_data.csv")
)
```
...
- `analysis_mode` controls the number of iterations and chains in the model run
  - `"debug"` is used for printing out the errors if the model does not sample
